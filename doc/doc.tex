...
The user is then able to move the robot on the map and new observations are
generated in real time. When presented with a new observation, the network
retrieves the most similar stored patterns. The robot's position is then
estimated based on the top-k most similar observations, effectively localizing
the robot by combining information from the best matching stored patterns. The
...
observations stored, observation dimensions, etc.
...
Wikipedia. \url{https://en.wikipedia.org/wiki/Modern_Hopfield_network}} improve
upon the classical version by using a different energy function that allows
storing exponentially more patterns while providing better retrieval accuracy.
The network works by storing patterns in memory during a learning phase, then
retrieving the most similar patterns when presented with a new query.
...
lower values allow it to consider a range of similar patterns. This is useful
when the robot's observation might match several nearby positions on the map.
...
\texttt{canvas\_state.py}, \texttt{canvas\_renderer.py} and
\texttt{robot\_state.py}, MHN is implemented inside \texttt{hopfield.py},
...
more vectors to influence the final position prediction.
...
black based on the distance to the obstacle, controlled by a configurable
...
Hopfield Networks require no iterative optimization when exact patterns are
stored.
...
allows the localization engine to combine information from multiple similar
observations, possibly providing more accurate localization when multiple
positions have similar visual features.
...
that position, queries the MHN, and records the attention weight assigned to
the best matching pattern. These attention weights represent the network's
confidence in its prediction. The resulting confidence values are interpolated
and displayed as a color-coded overlay (warmer colors -- high-confidence
regions).

The heatmap can be computed and displayed for each angle separately (changing
dynamically as the robot rotates), or averaged across all angles to show
overall confidence independent of orientation.

\paragraph{Energy heatmap} visualizes the Hopfield network's energy landscape across the map. For each
grid position, the application computes the energy $E(\mathbf{x})$ of the
observation at that location and angle using the Modern Hopfield Network energy
function:

\begin{equation}
    E(\mathbf{x}) = -\log\left(\sum_{i=1}^{N} \exp(\beta \sqrt{D} \cdot \text{sim}_i)\right)
\end{equation}

where $\text{sim}_i$ is the cosine similarity between the query observation and
the $i$-th stored pattern. Lower energy values indicate positions where the
observation fits well into the network's stored memory, representing stable
attractors in the energy landscape. The energy heatmap uses an inverted
colormap where brighter (warmer) colors indicate lower energy and thus better
matches.
...
\paragraph{Convergence} the program demonstrates the convergence to the most similar vector by
...
\paragraph{K-Means SGD Training} allows the network to learn a compact set of prototypes from the collected data
instead of storing all samples. The prototypes are initialized as a random
subset of the collected observations. Then, Stochastic Gradient Descent (SGD)
is used to iteratively update these prototypes to minimize the total energy of
the training set $\mathcal{X} = \{\mathbf{x}_1, \dots, \mathbf{x}_N\}$
consisting of all observations captured during the grid sampling phase. The
loss function is defined as the sum of energies $L = \sum_{\mathbf{x} \in
\mathcal{X}} E(\mathbf{x})$. By jointly optimizing both the visual patterns and
their associated positions using a K-Means style update rule, the prototypes
drift to the spatial centers of the visual clusters they represent, ensuring
somewhat accurate localization with significantly fewer stored patterns.

\section{Running the Program}

Unfortunately the installation process requires admin privileges.
...
After running the program, please start by importing a map using the Import Map
button and by training using sampling, exploring how the robot predicts the
location. Tweaking the beta parameter on the right side of the UI and the
Combine top-k matches will affect the final position prediction. Changing the
FOV will improve the localization accuracy. Training using SGD is less accurate
and may not yield as precise predictions as training using sampling.

\textcolor{red}{Unfortunately, the rendering inside the VM is a little broken; please see the
    image below on how the UI looks outside the VM, or fix it by modifying the VM
    settings to disable hardware acceleration in VirtualBox \textgreater VM
    Settings \textgreater Display \textgreater 3D Acceleration (\href{https://askubuntu.com/questions/1454571/some-application-guis-do-not-render-correctly-since-updating-to-22-04}{StackOverflow}).}

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{img/ui.png}
    \caption{\small{Cat represents the robot; network is trained using SGD; environment was sampled, 1000 random vectors were picked as initial memories, network was trained to best represent all sampled vectors using these 1000 memories; blue area represents what the robot sees; small red dots represent locations of learned memories; large red dots represent most similar memories; green dot is the weighted average of the 4 most similar memories; black lines connect most similar memories; heatmap represents the confidence of the network in its prediction, which is derived from the attention weights.}}
    \label{fig:ui}
\end{figure}
