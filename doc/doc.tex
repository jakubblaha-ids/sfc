...
\textbf{Beta ($\beta$)}: Inverse temperature (1.0–200.0). Higher values make the network focus on the best matches; lower values broaden the retrieval.
\textbf{Sample Stride}: Grid spacing for sample positions.
\textbf{Sample Rotations}: Number of orientations per position.
\textbf{Camera Samples}: Number of rays in observation strip. Determines the dimensionality of captured observations.
...
colors showing areas where localization is uncertain.
