...
Unfortunately, the rendering inside the VM is a little broken, so please see
the image below on how the UI should look and looks outside the VM.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{img/ui.png}
    \caption{Cat represents the robot; network is trained using SGD; environment was sampled, 1000 random vectors were picked as initial memories, network was trained to best represent all sampled vectors using these 1000 memories; blue area represents what the robot sees; small red dots represent locations of learned memories; large red dots represent most similar memories; green dot is the weighted average of the 4 most similar memories; black lines connect most similar memories; heatmap represents the confidence of the network in its prediction, which is derived from the attention weights.}
    \label{fig:ui}
\end{figure}
