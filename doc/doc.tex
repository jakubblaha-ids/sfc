...
The network uses the energy function
\begin{equation}
    E(x) = -\log\left(\sum_i \exp(\beta \cdot x^T \cdot \xi_i)\right)
\end{equation}
where $x$ is the query pattern, $\xi_i$ are stored patterns (normalized for
cosine similarity), and $\beta$ is the inverse temperature parameter.
...
applies softmax to obtain attention weights. The top-k patterns with highest
attention weights are returned along with their weights for position
estimation.
...
broader consideration of similar patterns.

\subsection{Observation capturing}

Visual observations are captured using a raycasting camera simulator
implemented in \texttt{CameraSimulator}. The robot's field of view is
represented as a cone defined by an angle (30-360°).
...
using attention weights computed from the energy function.
...
\begin{itemize}
    \item \textbf{Beta ($\beta$)}: Inverse temperature (1.0–200.0, default 50.0). Controls sharpness of pattern retrieval. Higher values make the network focus on the best matches; lower values broaden the retrieval.
    \item \textbf{Sample Stride}: Grid spacing for sample positions (default 50 pixels). Smaller values increase memory size but improve coverage density of the map.
    \item \textbf{Sample Rotations}: Number of orientations per position (default 16, equivalent to 22.5° steps). More rotations improve orientation accuracy during localization.
    \item \textbf{Camera Samples}: Number of rays in observation strip (fixed at 100). Determines the dimensionality of captured observations.
    \item \textbf{Blur Radius}: Gaussian blur applied to observations (0–5.0, default 4.0). Reduces noise sensitivity and improves localization robustness.
    \item \textbf{Field of View}: Camera viewing angle (30–360°, default 120°). Wider FOV captures more context.
    \item \textbf{Visibility Index}: Distance opacity factor (0.01–1.0, default 0.1). Lower values make distant objects fade more quickly, simulating limited visibility.
    \item \textbf{Top-k}: Number of matches to combine (1–20, default 1). Values greater than 1 smooth predictions by averaging multiple similar observations.
\end{itemize}
...
\subsubsection*{Confidence heatmap}
